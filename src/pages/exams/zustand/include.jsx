...
const GameCell = ({ index, value, isWinning, onClick }) => {
  return (
    <button
      className={`game-cell ${value ? "filled" : "empty"} ${
        isWinning ? "winning" : ""
    ...
    <Card className="game-status-card">
      <Space direction="vertical" size="middle" style={{ width: "100%" }}>
        ...
        <div className="game-stats">
          <Space size="large">
            <Text>
              <strong>총 수:</strong> {stats.totalMoves}
            </Text>
            <Text>
              <strong>X:</strong> {stats.xMoves}수
            </Text>
            <Text>
              <strong>O:</strong> {stats.oMoves}수
            </Text>
          </Space>
        </div>
      </Space>
...
const GameControls = () => {
  ...
  return (
    <Card className="game-controls-card">
      <Space size="middle">
        <Button type="primary" onClick={resetGame} size="large">
          🔄 새 게임
        </Button>
      ...
      <div className="history-moves">
        {moveHistory.map((move, index) => (
          <div key={index} className="history-move">
            ...
            />
            <Text>
              <strong>{move.player}</strong>가{" "}
              {Math.floor(move.position / 3) + 1}행 {(move.position % 3) + 1}
              열에
            </Text>
          </div>
        ))}
