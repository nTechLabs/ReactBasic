    ...
    <Card title="쿼리 파라미터 편집" size="small">
      <Space direction="vertical" style={{ width: '100%' }}>
        <Alert
          ...
          description={
            <span>
              <Text code>{'setParams({ foo, bar })'}</Text> 형태로 병합 업데이트. 빈 값은 자동 제거.
            </span>
          }
        />
        <Space>
          ...
          <Input
            ...
            style={{ width: 140 }}
          />
          <Button type="primary" icon={<ReloadOutlined />} onClick={() => setParams({ foo, bar })}>
            적용
          </Button>
          <Button icon={<ClearOutlined />} onClick={() => setParams({ foo: '', bar: '' })}>
            초기화
          </Button>
        </Space>
        <Text type="secondary" style={{ fontSize: 12 }}>
          빈 문자열 / null / undefined / 빈 배열은 자동 삭제
        </Text>
      </Space>
    </Card>
  );
...
const FilterPaginationDemo = () => {
  ...
  const categories = Array.isArray(all.category)
    ? all.category
    : all.category
      ? [all.category]
      : [];
    ...
    <Card title="필터 / 정렬 / 페이지네이션" size="small">
      <Space direction="vertical" style={{ width: '100%' }}>
        <Alert
          ...
          showIcon
          message="URL 기반 상태"
          description="필터/정렬/페이지 값을 URL로 관리하여 공유 & 북마크 가능"
        />
        <Row gutter={12}>
          <Col xs={24} md={8}>
            <Card
            ...
            >
              ...
              <Text type="secondary" style={{ fontSize: 12 }}>
                다중 선택 지원 (category=frontend&category=devops)
              </Text>
            </Card>
          </Col>
          <Col xs={24} md={8}>
          ...
          <Col xs={24} md={8}>
            <Card size="small" title="요약">
              <Space direction="vertical" size={4}>
                ...
                <Text>
                  페이지: <Tag color="geekblue">{page}</Tag>
                </Text>
              </Space>
              <Divider style={{ margin: '12px 0' }} />
              <Button block onClick={() => setParams({ sort: 'new', page: 1, category: [] })}>
                필터 초기화
              </Button>
            </Card>
          </Col>
  ...
  return (
    <Card title="실시간 검색 (디바운스)" size="small">
      <Space direction="vertical" style={{ width: '100%' }}>
        ...
        />
        <Text type="secondary" style={{ fontSize: 12 }}>
          500ms 정지 시 URL에 q 파라미터 반영
        </Text>
        <Space>
          <Tag color={typing ? 'orange' : 'blue'}>
            {typing ? '입력 중...' : `현재 q: ${all.q || '없음'}`}
          </Tag>
        </Space>
      </Space>
    </Card>
    ...
    <Card title="Boolean & 숫자 파라미터" size="small">
      <Space direction="vertical" style={{ width: '100%' }}>
        <Space>
          ...
          <Text>
            dark 모드: <Tag color={dark ? 'geekblue' : 'default'}>{dark ? 'ON' : 'OFF'}</Tag>
          </Text>
        </Space>
        <div>
        ...
        <Alert
          type="info"
          showIcon
          message="Boolean 값은 presence/값 조합"
          description="dark=true 나 삭제로 표현. 빈 문자열 전달 시 제거."
        />
      </Space>
    </Card>
  );
    ...
    <Card title="커스텀 훅 (useQueryParams)" size="small">
      <Space direction="vertical" style={{ width: '100%' }}>
        <Alert
          ...
          showIcon
          message="커스텀 훅 장점"
          description="읽기/쓰기 로직을 재사용하여 여러 컴포넌트에서 일관성 유지"
        />
        <Input.TextArea
          rows={3}
          value={localNote}
          onChange={(e) => setLocalNote(e.target.value)}
          placeholder="메모 -> URL note 파라미터"
        />
        <Space>
          <Button type="primary" onClick={() => setParams({ note: localNote })}>
            저장
          </Button>
          <Button
            onClick={() => {
...
const HistoryModeDemo = () => {
  ...
  const add = () => {
    const next = count + 1;
    setParams(
      { histCount: next, mode: navigateType === 'replace' ? 'replace' : 'push' },
      { replace: navigateType === 'replace' },
    );
  };
  return (
    <Card title="History 모드 (push vs replace)" size="small">
      <Space direction="vertical" style={{ width: '100%' }}>
        <Space>
          <Switch
            checked={navigateType === 'replace'}
            onChange={(v) => setParams({ mode: v ? 'replace' : 'push' })}
          />
  ...
  return (
    <Space direction="vertical" size="large" style={{ width: '100%' }}>
      <div style={{ textAlign: 'center' }}>
        <Title level={2}>
          <LinkOutlined style={{ color: '#0958d9' }} /> URLSearchParams 예제
        </Title>
        <Paragraph style={{ maxWidth: 760, margin: '0 auto', color: '#666' }}>
          브라우저 내장 <Text code>URLSearchParams</Text> 와 React Router{' '}
          <Text code>useLocation</Text>, <Text code>useNavigate</Text> 를 조합하여
      ...
      {/* 개념 섹션 */}
      <Card
        title="📚 URLSearchParams 기본 개념"
        style={{ backgroundColor: '#e6f4ff', border: '2px solid #1677ff', borderRadius: 12 }}
      >
        <Space direction="vertical" size="middle" style={{ width: '100%' }}>
          <div style={{ textAlign: 'center' }}>
            <Title level={4} style={{ color: '#1677ff', marginBottom: 8 }}>
              🎯 쿼리 문자열을 읽고/쓰기 위한 표준 Web API
            </Title>
            <Text type="secondary">
              키-값 쌍 검색 파라미터를 다루며 여러 동일 키(다중 값)도 지원합니다
            </Text>
          </div>
      ...
      <Card title="📘 URLSearchParams 패턴 가이드" style={{ background: '#f6f6f6' }}>
        <Row gutter={[16, 16]}>
          ...
          <Col xs={24} sm={12} md={6}>
            <Card size="small">
              ...
              <Paragraph style={{ fontSize: 12, margin: '8px 0 0 0' }}>
        ...
        >
          ...
          <Paragraph style={{ margin: '4px 0 0 0', fontSize: 12 }}>
            useMemo로 location.search 파싱 결과를 메모이제이션 하고, setParams 호출을 최소화하여
            불필요한 리렌더를 줄입니다.
          </Paragraph>
        </div>
      </Card>
