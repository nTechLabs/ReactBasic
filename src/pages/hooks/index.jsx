...
import { ApiOutlined, FunctionOutlined, ArrowRightOutlined, CodeOutlined } from '@ant-design/icons';
      ...
      <div style={{ textAlign: 'center', marginBottom: '32px' }}>
        ...
        >
          <Paragraph style={{ fontSize: '15px', lineHeight: '1.6', marginBottom: '16px' }}>
            React Hooks는 React 16.8에서 도입된 혁신적인 기능으로, 함수형 컴포넌트에서도 클래스
            컴포넌트의 상태(state)와 생명주기 메서드(lifecycle methods)를 사용할 수 있게 해주는
            특별한 함수들입니다. 'use'로 시작하는 함수들로, React의 다양한 기능을 "갈고리(hook)"
            처럼 연결하여 사용할 수 있습니다.
          </Paragraph>
          ...
          <Paragraph style={{ fontSize: '14px', lineHeight: '1.7', textAlign: 'justify' }}>
            React Hooks의 가장 큰 장점은 뛰어난 코드 재사용성입니다. 커스텀 훅(Custom Hook)을 통해
            복잡한 상태 로직이나 비즈니스 로직을 별도의 함수로 분리하여 여러 컴포넌트에서 공유할 수
            있습니다. 예를 들어, API 호출 로직, 폼 검증 로직, 로컬 스토리지 관리 등을 한 번 작성하면
            프로젝트 전체에서 재사용이 가능하며, 이는 개발 효율성을 크게 향상시키고 코드 중복을
            현저히 줄여줍니다. 또한 간결하고 읽기 쉬운 코드 작성이 가능합니다. 클래스 컴포넌트에서
            필요했던 constructor, this 바인딩, render 메서드 등의 복잡한 보일러플레이트 코드가 전혀
            필요 없습니다. 함수형 컴포넌트와 함께 사용하면 더 직관적이고 간결한 코드 작성이
            가능하며, JavaScript의 함수형 프로그래밍 패러다임과 자연스럽게 결합되어 코드의 가독성과
            유지보수성이 현저히 향상됩니다. React Hooks는 효과적인 관심사 분리를 가능하게 합니다.
            관련된 상태와 로직을 하나의 훅으로 묶어서 관리할 수 있어 코드의 응집도가 크게
            높아집니다. 클래스 컴포넌트에서는 componentDidMount, componentDidUpdate,
            componentWillUnmount 등의 생명주기 메서드에 서로 다른 관심사가 섞여있어 코드를 이해하고
            관리하기 어려웠습니다. 반면 Hooks를 사용하면 각각의 기능별로 로직을 분리하여 관리할 수
            있고, 이는 단일 책임 원칙을 준수하여 코드의 유지보수성과 테스트 용이성을 크게
            개선시킵니다. 마지막으로 강력한 성능 최적화 기능을 제공합니다. useMemo와 useCallback
            등의 성능 최적화 훅을 활용하여 불필요한 연산과 리렌더링을 효과적으로 방지할 수 있습니다.
            의존성 배열을 통해 정확히 언제 재계산이나 리렌더링이 필요한지를 세밀하게 제어할 수
            있으며, 이는 특히 대규모 애플리케이션에서 사용자 경험을 크게 향상시킵니다. React.memo와
            함께 사용하면 컴포넌트 최적화를 더욱 정교하게 수행할 수 있어, 복잡한 UI에서도 최적의
            성능을 유지할 수 있습니다.
          </Paragraph>
        </div>
      </div>
