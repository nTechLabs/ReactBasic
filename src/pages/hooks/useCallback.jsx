          ...
          description={
            <div>
              ...
              <ul style={{ marginTop: '8px', marginBottom: '8px' }}>
                <li><Text code>handleCalculateWithCallback = useCallback(fn, [count])</Text> - count 변경시에만 함수 재생성</li>
                <li><Text code>ExpensiveComponent = memo()</Text> - props 변경시에만 리렌더링</li>
                <li><Text code>onCalculate={`{handleCalculateWithCallback}`}</Text> - 메모이제이션된 함수 전달</li>
                <li>기타 값 변경시에도 자식 컴포넌트는 리렌더링되지 않음</li>
              </ul>
              <Text type="secondary" style={{ fontSize: '12px' }}>
                💡 핵심: useCallback과 React.memo를 함께 사용하여 불필요한 리렌더링을 방지합니다. 
                콘솔에서 "기타 값 증가" 버튼을 클릭해도 ExpensiveComponent가 리렌더링되지 않는 것을 확인하세요.
              </Text>
            </div>
          }
          type="info"
        ...
        <Alert
          ...
          description={
            <div>
              ...
              <ul style={{ marginTop: '8px', marginBottom: '8px' }}>
                <li><Text code>handleSearch = useCallback(fn, [mockData])</Text> - 검색 로직 메모이제이션</li>
                <li><Text code>debouncedSearch = useCallback(fn, [handleSearch])</Text> - 디바운스 함수 메모이제이션</li>
                <li>검색어 변경시마다 새로운 함수가 생성되는 것을 방지</li>
                <li>300ms 디바운스로 불필요한 API 호출 방지</li>
              </ul>
              <Text type="secondary" style={{ fontSize: '12px' }}>
                💡 핵심: useCallback으로 검색 함수를 메모이제이션하여 디바운싱이 정확히 작동하도록 합니다. 
                함수가 매번 새로 생성되면 디바운스 타이머가 제대로 작동하지 않을 수 있습니다.
              </Text>
            </div>
          }
          type="info"
        ...
        <Alert
          ...
          description={
            <div>
              ...
              <ul style={{ marginTop: '8px', marginBottom: '8px' }}>
                <li><Text code>handleToggle = useCallback(fn, [])</Text> - 할일 토글 함수 메모이제이션</li>
                <li><Text code>handleDelete = useCallback(fn, [])</Text> - 할일 삭제 함수 메모이제이션</li>
                <li><Text code>handleAdd = useCallback(fn, [newTodo, todos])</Text> - 할일 추가 함수 메모이제이션</li>
                <li><Text code>TodoItem = memo()</Text> - 개별 할일 아이템 메모이제이션</li>
              </ul>
              <Text type="secondary" style={{ fontSize: '12px' }}>
                💡 핵심: useCallback으로 이벤트 핸들러를 메모이제이션하여 개별 TodoItem이 불필요하게 리렌더링되지 않도록 합니다. 
                하나의 할일을 변경해도 다른 할일 아이템들은 리렌더링되지 않습니다.
              </Text>
            </div>
          }
          type="success"
          ...
          <Space direction="vertical" size="middle" style={{ width: '100%' }}>
            ...
            <div style={{ textAlign: 'center', marginBottom: '16px' }}>
              <Title level={4} style={{ color: '#fa8c16', marginBottom: '8px' }}>
                🎯 useCallback은 함수를 메모이제이션하여 불필요한 리렌더링을 방지하는 React Hook입니다
              </Title>
              <Text type="secondary">
                의존성 배열의 값이 변경될 때만 함수를 새로 생성하고, 그렇지 않으면 이전 함수를 재사용합니다
              </Text>
            </div>
