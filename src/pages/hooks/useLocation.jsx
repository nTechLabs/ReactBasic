    ...
    <Card title="현재 Location 정보" size="small">
      <Space direction="vertical" style={{ width: '100%' }}>
        ...
        <Alert
          type="info"
          showIcon
          message="location 객체는 불변"
          description="pathname/search/hash 변경 시 새 객체가 생성되므로 useEffect 의존성에 그대로 사용 가능"
        />
      </Space>
    </Card>
  );
  ...
  return (
    <Card title="탭과 URL 동기화" size="small">
      ...
      <Alert
        ...
        type="success"
        showIcon
        message="현재 탭이 URL 마지막 세그먼트와 동기화됨"
      />
    </Card>
  );
};
...
const LocationWatcherDemo = () => {
  ...
  useEffect(() => {
    setHistory((h) => [
      ...h.slice(-9),
      { ts: Date.now(), path: location.pathname + location.search + location.hash },
    ]);
  }, [location]);
    ...
    <Card title="쿼리 파라미터 편집" size="small">
      <Space>
        ...
        <Input
          ...
          style={{ width: 120 }}
        />
        <Button type="primary" onClick={apply} icon={<ReloadOutlined />}>
          적용
        </Button>
      </Space>
      <Paragraph style={{ marginTop: 12 }} type="secondary">
...
const ConditionalRenderDemo = () => {
  ...
  return (
    <Card title="조건부 렌더링 (pathname 검사)" size="small">
      {isSettings ? (
        <Alert type="info" message="/settings 경로입니다" showIcon />
      ) : (
        <Alert type="warning" message="settings 경로가 아닙니다" showIcon />
      )}
      <Paragraph style={{ marginTop: 12 }} type="secondary">
        pathname.includes("settings") 결과에 따라 UI 변경
      </Paragraph>
    </Card>
  );
};
  ...
  return (
    <div style={{ padding: 24 }}>
      <div style={{ textAlign: 'center', marginBottom: 32 }}>
        <Title level={1}>
          <CompassOutlined style={{ color: '#13c2c2' }} /> useLocation Hook 예제
        </Title>
        <Paragraph style={{ fontSize: 16, color: '#666' }}>
          현재 URL 상태를 활용한 다양한 패턴을 학습하세요
        </Paragraph>
      </div>
        ...
        >
          <Row gutter={[16, 16]}>
            <Col xs={24} md={12}>
              ...
              <pre
                style={{ background: '#f6f6f6', padding: 12, fontSize: 12 }}
...
const foo = params.get('foo')`}</pre>
            ...
            <Col xs={24} md={12}>
              ...
              <pre
                style={{ background: '#f6f6f6', padding: 12, fontSize: 12 }}
              >{`const active = location.pathname.split('/').pop()`}</pre>
            </Col>
            <Col xs={24} md={12}>
  ...
  // 경로 변경 시 실행
          ...
          <Alert
            ...
            showIcon
            message="주의"
            description="location 객체 자체는 매 변경마다 새로 생성되니 필요한 속성만 의존성 배열에 넣어 불필요한 useEffect 실행을 줄일 수 있음"
          />
        </Card>
      </Space>
    </div>
