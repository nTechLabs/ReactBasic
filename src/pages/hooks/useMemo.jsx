          ...
          description={
            <div>
              ...
              <ul style={{ marginTop: '8px', marginBottom: '8px' }}>
                <li>
                  <Text code>{'useMemo(() => heavyCalculation(), [count, multiplier])'}</Text> -
                  계산 결과 메모이제이션
                </li>
              ...
              <Text type="secondary" style={{ fontSize: '12px' }}>
                💡 핵심: 무거운 계산을 메모이제이션하여 불필요한 재계산을 방지합니다. "기타 값 증가"
                버튼을 클릭해도 계산이 다시 실행되지 않습니다.
              </Text>
            </div>
          }
          type="success"
              ...
              <Space direction="vertical" style={{ width: '100%' }}>
                ...
                <div>
                  ...
                  <div style={{ marginTop: 8 }}>
                    <Button onClick={() => setOtherValue((v) => v + 1)} icon={<ReloadOutlined />}>
                      기타 값 증가
                    </Button>
                  </div>
                </div>
    ...
    <Card title="상품 필터링 및 정렬 (useMemo 최적화)">
      <Space direction="vertical" size="middle" style={{ width: '100%' }}>
            ...
            <div>
              ...
              <ul style={{ marginTop: '8px', marginBottom: '8px' }}>
                <li>
                  <Text code>
                    {
                      'filteredProducts = useMemo(() => products.filter(...), [products, searchTerm, minPrice, maxPrice])'
                    }
                  </Text>
                </li>
                <li>
                  <Text code>
                    {
                      'sortedProducts = useMemo(() => [...filteredProducts].sort(...), [filteredProducts, sortBy])'
                    }
                  </Text>
                </li>
                <li>
                  <Text code>
                    {'statistics = useMemo(() => calculateStats(...), [sortedProducts])'}
                  </Text>
                </li>
                <li>각 단계별로 필요한 의존성만 포함하여 효율적인 재계산</li>
              </ul>
              <Text type="secondary" style={{ fontSize: '12px' }}>
                💡 핵심: 필터링→정렬→통계 계산을 체인으로 연결하여 각각 독립적으로
                메모이제이션합니다. 강제 리렌더링 시에도 불필요한 계산은 수행되지 않습니다.
              </Text>
            </div>
          }
          type="info"
        ...
        <Row gutter={[16, 16]}>
          ...
          <Col xs={8}>
            <Statistic title="평균 가격" value={statistics.averagePrice} precision={2} prefix="$" />
          </Col>
          <Col xs={8}>
            <Statistic
              title="평균 평점"
    ...
    <Card title="참조 동등성과 React.memo (useMemo + useCallback)">
      <Space direction="vertical" size="middle" style={{ width: '100%' }}>
        ...
        <Alert
          ...
          description={
            <div>
              ...
              <ul style={{ marginTop: '8px', marginBottom: '8px' }}>
                <li>
                  <Text code>{'dataWithMemo = useMemo(() => ({count, timestamp}), [count])'}</Text>{' '}
                  - 객체 참조 안정화
                </li>
                <li>
                  <Text code>handleUpdate = useCallback(fn, [])</Text> - 함수 참조 안정화
                </li>
                <li>React.memo와 함께 사용하여 불필요한 리렌더링 방지</li>
                <li>count 변경시에만 새로운 객체 생성</li>
              </ul>
              <Text type="secondary" style={{ fontSize: '12px' }}>
                💡 핵심: useMemo로 객체의 참조 동등성을 유지하여 자식 컴포넌트의 불필요한 리렌더링을
                방지합니다. 메시지 변경 시에도 ChildComponent가 리렌더링되지 않습니다.
              </Text>
            </div>
          }
          type="warning"
        ...
        <Row gutter={[16, 16]}>
            ...
            <Card size="small" title="컨트롤 패널">
              <Space direction="vertical" style={{ width: '100%' }}>
                ...
                <div>
                  ...
                  <div style={{ marginTop: 8 }}>
                    <Button onClick={() => setMessage(`업데이트됨 ${Date.now()}`)}>
                      메시지 변경 (count에 무관)
                    </Button>
                  </div>
                </div>
          ...
          <Col xs={24} md={12}>
            <ChildComponent data={dataWithMemo} onUpdate={handleUpdate} />
          </Col>
        </Row>
...
const UseMemoPage = () => {
          ...
          <Space direction="vertical" size="middle" style={{ width: '100%' }}>
            ...
            <div style={{ textAlign: 'center', marginBottom: '16px' }}>
              <Title level={4} style={{ color: '#52c41a', marginBottom: '8px' }}>
                🎯 useMemo는 계산 결과를 메모이제이션하여 불필요한 재계산을 방지하는 React
                Hook입니다
              </Title>
              <Text type="secondary">
                의존성 배열의 값이 변경될 때만 계산을 다시 수행하고, 그렇지 않으면 이전 결과를
                재사용합니다
              </Text>
            </div>
  ...
  return calculate()
                    ...
                    <div style={{ marginTop: '16px', textAlign: 'center' }}>
                      <Text style={{ fontSize: '13px', color: '#8c8c8c', lineHeight: '1.6' }}>
                        <strong>의존성 배열의 패턴 3가지</strong>
                        <br />
                        1. 컴포넌트 생성시 한번만 실행
                        <br />
                        2. 의존성 value변경시에만 재계산
                        <br />
                        3. 매 렌더링마다 재계산
                      </Text>
                    </div>
    ...
    item.name.toLowerCase()
      .includes(searchTerm.toLowerCase())
                          ...
                          >
                            ...
                            배열 순회와 문자열 변환을 메모이제이션. items나 searchTerm 변경시에만
                            필터링하여 렌더링 성능 개선.
                          </div>
                        </div>
                      </Col>
  ...
  return [...data].sort((a, b) => {
    ...
    return a[sortBy] - b[sortBy]
                          ...
                          >
                            <Text strong style={{ fontSize: '9px', color: '#fa8c16' }}>
                              💡 장점:
                            </Text>
                            배열 복사 및 정렬 연산을 메모이제이션. data나 sortBy 기준 변경시에만
                            재정렬하여 UI 반응성 향상.
                          </div>
                        </div>
                      </Col>
  ...
  onUpdate: handleUpdate
                          ...
                          >
                            <Text strong style={{ fontSize: '9px', color: '#722ed1' }}>
                              💡 장점:
                            </Text>
  ...
  return (
    <div>
      ...
      {/* otherState 변경시 Child 리렌더링 안됨 */}
                            ...
                            >
                              <Text strong style={{ fontSize: '8px' }}>
                                ⚡ 핵심 장점:
                              </Text>
                              otherState 변경시 ChildComponent가 리렌더링되지 않음. useMemo 없이는
                              매번 새 객체가 생성되어 React.memo 효과 무력화.
                            </div>
                          </div>
                        </div>
                      </Col>
